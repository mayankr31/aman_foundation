...
export default function RolePermissionsModal({ isOpen, onClose, role, isDarkMode, token }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      initializeLocalState(permsData.data || [], rolePermsData.data || []);
    } catch (err) {
  ...
  const initializeLocalState = (perms, rolePerms) => {
    const state = {};
    const rolePermsMap = {};
    ...
    perms.forEach(p => {
      state[p.id] = {
        hasPermission: !!rolePermsMap[p.id],
        changed: false,
      };
    });
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const permissionIds = Object.entries(localState)
        .filter(([_, state]) => state.hasPermission)
        .map(([permId]) => permId);
