...
export default function AfterSchoolCentreProfileDetail() {
  const { id } = useParams();
  ...
  const loadCentre = useCallback(async () => {
    try {
      const res = await fetch(`/api/after-school-centres/${id}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
    ...
    } catch (err) { console.error("Failed to load centre:", err); }
  ...
  useEffect(() => {
    ...
    if (modal === "students") {
      fetch("/api/after-school-students", { headers: h }).then(r => r.json()).then(j => { if (j.success) setAllStudents(j.data); });
    }
    if (modal === "fellows") {
  ...
  const loadExistingAttendance = useCallback(async (date) => {
    if (!date) return;
    try {
      const res = await fetch(`/api/after-school-centres/${id}/attendance?date=${date}`, { headers: authHeaders() });
      const json = await res.json();
      if (json.success && json.data.length > 0) {
        ...
        setAttendanceSelection(new Set(centre?.students?.map(s => s.id) || []));
  ...
  }, [id, authHeaders, centre?.students]);

  const loadCalendarData = useCallback(async () => {
    try {
      const res = await fetch(`/api/after-school-centres/${id}/attendance/calendar?month=${currentMonth}&year=${currentYear}`, { headers: authHeaders() });
      const json = await res.json();
      if (json.success) {
        const dataMap = {};
  ...
  async function handleEditSave(e) {
    ...
    try {
      const res = await fetch(`/api/after-school-centres/${id}`, { method: "PATCH", headers: authHeaders(), body: JSON.stringify(editForm) });
      const json = await res.json();
      if (json.success) { await loadCentre(); setModal(null); toast.success("Profile saved"); }
  ...
  async function handleAssignStudent(studentId) {
    await fetch(`/api/after-school-centres/${id}/students`, { method: "POST", headers: authHeaders(), body: JSON.stringify({ studentId }) });
    await loadCentre();
  }

  async function handleRemoveStudent(studentId) {
    if (!confirm("Remove this student from the centre?")) return;
    await fetch(`/api/after-school-centres/${id}/students`, { method: "DELETE", headers: authHeaders(), body: JSON.stringify({ studentId }) });
    await loadCentre();
  }

  async function handleAssignFellow(fellowId) {
    await fetch(`/api/after-school-centres/${id}/fellows`, { method: "POST", headers: authHeaders(), body: JSON.stringify({ fellowId }) });
    await loadCentre();
  }

  async function handleRemoveFellow(fellowId) {
    if (!confirm("Remove this fellow from the centre?")) return;
    await fetch(`/api/after-school-centres/${id}/fellows`, { method: "DELETE", headers: authHeaders(), body: JSON.stringify({ fellowId }) });
    await loadCentre();
  }

  async function handleAssignManager(userId) {
    const res = await fetch(`/api/after-school-centres/${id}/program-managers`, { method: "POST", headers: authHeaders(), body: JSON.stringify({ userId }) });
    const json = await res.json();
  ...
  async function handleRemoveManager(userId) {
    if (!confirm("Remove this Program Manager from the centre?")) return;
    const res = await fetch(`/api/after-school-centres/${id}/program-managers`, { method: "DELETE", headers: authHeaders(), body: JSON.stringify({ userId }) });
    const json = await res.json();
  ...
  async function handleAssignProgram(programId) {
    await fetch(`/api/after-school-centres/${id}/programs`, { method: "POST", headers: authHeaders(), body: JSON.stringify({ programId }) });
    await loadCentre();
  }

  async function handleRemoveProgram(programId) {
    if (!confirm("Remove this program from the centre?")) return;
    await fetch(`/api/after-school-centres/${id}/programs`, { method: "DELETE", headers: authHeaders(), body: JSON.stringify({ programId }) });
    await loadCentre();
  }
  ...
  async function handleConfirmAttendance() {
    setSaving(true);
    try {
      const studentStatuses = (centre?.students || []).map(s => ({
        studentId: s.id,
      ...
      if (includeLearningAssessment) {
        body.learningAssessments = (centre?.students || []).map(s => ({
          studentId: s.id,
          canRead: !!learningAssessmentMap[s.id]
        }));
      }
      if (includeHomework) {
        body.homework = (centre?.students || []).map(s => ({
          studentId: s.id,
          status: homeworkMap[s.id] || "NO_HOMEWORK"
        }));
      }
      const res = await fetch(`/api/after-school-centres/${id}/attendance`, {
        method: "POST",
        headers: authHeaders(),
  ...
  if (!centre) return <div className="p-8 text-center text-on-surface-variant font-medium">After School Centre not found</div>;

  const enrolledStudents = centre.students || [];
  const assignedFellows = centre.fellows || [];
  const assignedPrograms = centre.programs || [];
  const assignedManagers = centre.programManagers || [];
  const isAdmin = user?.roleName === "ADMIN";
  const { totalEnrolled = 0, genderRatio = {} } = centre;
  ...
  const centreStudentIds = new Set(enrolledStudents.map(s => s.id));
  const centreFellowIds = new Set(assignedFellows.map(f => f.fellowId));
  const centreProgramIds = new Set(assignedPrograms.map(p => p.programId));
  const centreManagerIds = new Set(assignedManagers.map(m => m.userId));
  ...
  return (
    <div className="p-6 md:p-10 pb-24 overflow-x-hidden max-w-7xl mx-auto w-full">
      {/* Back */}
      <Link href="/education/after-school-centres" className="flex items-center gap-2 text-slate-500 hover:text-teal-600 transition-colors mb-6 group w-fit">
        ...
        <span className="text-[10px] font-bold uppercase tracking-widest font-sans">Back to After School Centres Directory</span>
      </Link>
        ...
        <div className="flex flex-col md:flex-row gap-6 items-start relative z-10">
          {centre.img ? (
            <div className="w-24 h-24 md:w-32 md:h-32 rounded-xl bg-surface-container overflow-hidden shrink-0 border-4 border-surface shadow-md">
              <img alt="After School Centre" className="w-full h-full object-cover" src={centre.img} />
            </div>
          ) : (
          ...
          <div className="pt-2">
            <div className="flex flex-wrap items-center gap-3 mb-2">
              <h2 className="text-3xl font-headline font-black text-on-surface capitalize leading-tight">{centre.name}</h2>
              <span className={`text-xs font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${centre.status === "Active" ? "bg-primary-fixed text-on-primary-fixed" : "bg-surface-container text-on-surface-variant"}`}>
                {centre.status}
              </span>
            </div>
            <p className="text-on-surface-variant font-medium mb-3 flex items-center gap-2">
              <span className="material-symbols-outlined text-sm text-primary">location_on</span>
              {centre.location} • Regional Impact Partner
            </p>
            <div className="flex flex-col gap-2 mt-2 text-xs font-medium text-slate-500 font-sans">
              <div className="flex flex-wrap gap-4">
                <div><span className="font-bold text-on-surface">Coordinator:</span> {centre.coordinatorName || "—"}</div>
              </div>
              <div className="flex flex-wrap gap-4">
                <div><span className="font-bold text-on-surface">Email:</span> {centre.email || "—"}</div>
                <span className="w-1 h-1 bg-surface-container-highest rounded-full self-center"></span>
                <div><span className="font-bold text-on-surface">Phone:</span> {centre.phone || "—"}</div>
              </div>
              {centre.address && <div><span className="font-bold text-on-surface">Address:</span> {centre.address}</div>}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          {/* Enrolled After School Students */}
          <div className="bg-surface-container-lowest rounded-xl p-6 shadow-ambient border border-outline-variant/10">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-headline font-bold text-xl text-on-surface flex items-center gap-2">
                <span className="material-symbols-outlined text-primary">groups</span>
                Enrolled After School Students ({totalEnrolled})
              </h3>
                    ...
                    {enrolledStudents.map(s => (
                      <tr key={s.id} className="border-b border-surface-container last:border-none hover:bg-surface-container-low/50 transition-colors">
                        <td className="py-3 px-3">
                          <Link href={`/education/after-school-students/${s.id}`} className="font-semibold text-primary hover:underline">{s.name}</Link>
                        </td>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          {/* Centre Metrics */}
          <div className="bg-surface-container-lowest rounded-xl p-6 shadow-ambient border border-outline-variant/10">
            <h3 className="font-headline font-bold text-base text-on-surface mb-6">Centre Metrics</h3>
            <div className="space-y-4 font-sans text-sm">
              ...
              <div className="flex justify-between py-2 border-b border-surface-container">
                <span className="text-on-surface-variant font-medium">Enrolment Goal</span>
                <span className="font-bold text-on-surface">{centre.goal} Students</span>
              </div>
              <div className="flex justify-between py-2 border-b border-surface-container">
      ...
      {/* Edit Centre Profile */}
      {modal === "edit" && (
        <Modal title="Edit Centre Profile" onClose={() => setModal(null)}>
          <form onSubmit={handleEditSave} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <InputField label="Centre Name" name="name" value={editForm.name} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))} required />
              <InputField label="Coordinator Name" name="coordinatorName" value={editForm.coordinatorName} onChange={e => setEditForm(f => ({ ...f, coordinatorName: e.target.value }))} />
      ...
      {modal === "students" && (
        <Modal title="Manage Students" onClose={() => setModal(null)}>
          <p className="text-sm text-on-surface-variant mb-4">Assign or remove after school students from this centre. Unassigned students are highlighted.</p>
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {filteredStudents.map(s => {
              const isAssigned = centreStudentIds.has(s.id);
              return (
                <div key={s.id} className={`flex items-center justify-between p-3 rounded-lg border transition-colors ${isAssigned ? "border-primary/30 bg-primary/5" : "border-outline-variant hover:bg-surface-container"}`}>
                  <div>
                    <p className="font-semibold text-on-surface text-sm">{s.name}</p>
                    <p className="text-xs text-on-surface-variant">{s.studentId} • {s.grade} • {s.centreId ? (isAssigned ? "This Centre" : "Other Centre") : "Unassigned"}</p>
                  </div>
                  <button
      ...
      {modal === "fellows" && (
        <Modal title="Manage Fellows" onClose={() => setModal(null)}>
          <p className="text-sm text-on-surface-variant mb-4">Assign or remove fellows from this centre. A fellow can be assigned to multiple centres.</p>
          <input type="text" placeholder="Search fellows..." value={searchQ} onChange={e => setSearchQ(e.target.value)}
            className="w-full px-4 py-2 border border-outline-variant rounded-lg bg-surface text-sm text-on-surface focus:outline-none focus:border-primary mb-4" />
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {filteredFellows.map(f => {
              const isAssigned = centreFellowIds.has(f.id);
              return (
      ...
      {modal === "programs" && (
        <Modal title="Manage Programs" onClose={() => setModal(null)}>
          <p className="text-sm text-on-surface-variant mb-4">Assign or remove programs from this centre.</p>
          <input type="text" placeholder="Search programs..." value={searchQ} onChange={e => setSearchQ(e.target.value)}
            className="w-full px-4 py-2 border border-outline-variant rounded-lg bg-surface text-sm text-on-surface focus:outline-none focus:border-primary mb-4" />
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {filteredPrograms.map(p => {
              const isAssigned = centreProgramIds.has(p.id);
              return (
      ...
      {modal === "managers" && (
        <Modal title="Manage Program Managers" onClose={() => setModal(null)}>
          <p className="text-sm text-on-surface-variant mb-4">Assign or remove Program Managers for this centre. Program Managers can manage the fellows and students of this centre.</p>
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            ...
            {filteredManagers.map(m => {
              const isAssigned = centreManagerIds.has(m.id);
              return (
